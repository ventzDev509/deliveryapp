      ...
      <nav className="hidden md:flex flex-col w-64 h-screen bg-white border-r border-gray-200 p-6 justify-between fixed left-0 top-0 dark:bg-zinc-950 dark:border-zinc-900 transition-colors duration-300">
        <div>
          <div className="flex items-center gap-2 mb-10 px-2">
            <div className="w-8 h-8 bg-amber-500 rounded-lg flex items-center justify-center text-white font-bold">D</div>
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex items-center gap-4 px-4 py-3 rounded-xl transition-all ${isActive
                    ? 'bg-orange-50 text-orange-600 dark:bg-amber-500/10 dark:text-amber-400'
      ...
      <nav className="md:hidden bottom-nav fixed bottom-0 left-0 w-full bg-white border-t border-gray-100 flex justify-around items-center p-3 z-50 dark:bg-zinc-950 dark:border-zinc-900 transition-colors duration-300">
        {menuItems.map((item) => (
          <NavLink
            ...
            to={item.path}
            className={({ isActive }) =>
              `flex flex-col items-center gap-1 p-2 ${isActive ? 'text-amber-500 dark:text-amber-400' : 'text-gray-400 dark:text-zinc-500'
              }`
            }
          >
            <item.icon size={22} />
