...
const MenuPage = () => {
  ...
  const loadRestaurantData = async () => {
    if (user?.id) {
      ...
      if (data && data.menus) {
        // Map done ki soti nan backend la pou yo korespondan ak koòdone Dish la
  ...
  // Chanje estati disponiblite (Disponib / Fini) dirèkteman nan backend la
  const toggleAvailability = async (id: string) => {
    ...
    try {
      ...
      // Mete ajou nan eta lokal la tou
  ...
  return (
    <div className="w-full flex flex-col gap-6 text-gray-900 dark:text-zinc-100 transition-colors duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          ...
          <button onClick={() => handleOpenDrawer(null)} className="flex items-center gap-2 px-4 py-2.5 bg-orange-500 text-white rounded-2xl text-xs font-bold transition-all shadow-md shadow-orange-500/10 active:scale-95 hover:bg-orange-600">
            <Plus size={16} />
      ...
      {/* 2. FILTRE KATEGORI */}
      <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-none border-b border-gray-100 dark:border-zinc-800/60">
        {/* Bouton Tout */}
        <button
          onClick={() => setActiveCategory('All')}
          className={`px-4 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center gap-2 border whitespace-nowrap ${activeCategory === 'All'
        ...
        {categories.map((cat) => {
          ...
          return (
            <button
              ...
              className={`px-4 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center gap-2 border whitespace-nowrap ${isSelected
                  ? 'bg-gray-950 text-white border-gray-950 shadow-lg shadow-gray-950/10 dark:bg-zinc-100 dark:text-zinc-950 dark:border-zinc-100'
                  : 'bg-white text-gray-500 border-gray-100 dark:bg-zinc-900 dark:text-zinc-400 dark:border-zinc-800'
                }`}
            >
          ...
          <AnimatePresence mode="popLayout">
            {filteredDishes.map((dish) => (
              <motion.div
                ...
                className={`bg-white rounded-2xl border flex flex-col justify-between overflow-hidden transition-colors duration-200 ${dish.isAvailable
              ...
              >
                ...
                <div className="p-4 flex gap-4">
                  <div className="w-20 h-20 md:w-24 md:h-24 rounded-xl overflow-hidden bg-gray-100 dark:bg-zinc-800 flex-shrink-0 relative">
                    <img
                      src={
                  ...
                  <div className="flex-1 min-w-0 flex flex-col justify-between">
                    <div className="flex items-start justify-between gap-2">
                      <h3 className="text-sm font-black text-gray-900 dark:text-zinc-50 truncate">{dish.name}</h3>
                      <span className="text-xs font-black text-orange-600 bg-orange-50 dark:bg-orange-950/40 dark:text-orange-400 px-2 py-0.5 rounded-lg">{dish.price.toFixed(2)} gds</span>
                    </div>
                    <p className="text-[11px] text-gray-400 dark:text-zinc-400 mt-1 line-clamp-2">{dish.description}</p>
                    {/* Ti liy pou statistik ak tan preparasyon */}
                    <div className="flex items-center gap-3 mt-2">
                      <div className="text-[10px] text-gray-400 dark:text-zinc-500 font-bold">🔥 kòmande {dish.salesCount} fwa</div>
                      <div className="flex items-center gap-1 text-[10px] bg-gray-100 dark:bg-zinc-800 text-gray-600 dark:text-zinc-400 px-1.5 py-0.5 rounded-md font-bold">
                ...
                <div className="px-4 py-3 bg-gray-50/50 dark:bg-zinc-900/50 border-t border-gray-100 dark:border-zinc-800/60 flex items-center justify-between gap-4">
                  <button onClick={() => dish.id && toggleAvailability(dish?.id)} className="flex items-center gap-2 text-left cursor-pointer">
                    {dish.isAvailable ? <ToggleRight className="text-emerald-500" size={24} /> : <ToggleLeft className="text-gray-300 dark:text-zinc-600" size={24} />}
                    <span className={`text-[11px] font-bold ${dish.isAvailable ? 'text-emerald-700 dark:text-emerald-400' : 'text-gray-400 dark:text-zinc-500'}`}>{dish.isAvailable ? 'Disponib' : 'Fini'}</span>
                  </button>
